...
## Acceptance Criteria
- [x] `packages/core/src/` groups files into an explicit domain layer (entities/value objects: `Nonogram`, `NonogramClues`, grid) separated from infrastructure/application concerns (rendering, persistence interfaces, reMarkable Cloud integration)
- [x] Domain modules (grid, clue computation, line solver) have no import dependency on infrastructure modules (PDF/SVG rendering, credential/nonogram stores, reMarkable client)
- [x] Naming of domain types and functions matches the ubiquitous language already documented in `.vibe/glossary.md`
- [x] CLAUDE.md's review agents table is updated to mark `vibe:review-ddd` active, and `.vibe/index.md` is refreshed via `/vibe:sync` to reflect the new structure

...
## Acceptance Criteria
- [x] User can trigger an import of a puzzle from nonograms.org (e.g. by pasting its URL or id) through the web app
- [x] The system fetches the puzzle's grid from the source and creates a saved nonogram whose row/column clues are computed the same way as for a manually-drawn grid, never trusted as-is from the source
- [x] An imported nonogram appears in the saved nonograms list and is immediately usable for preview, PDF download, and send-to-reMarkable, exactly like a manually-drawn one
- [x] A clear error is shown if the source is unreachable, the puzzle id/URL is invalid, or the fetched grid can't be parsed
...
**Implemented (2026-07-11):** reopened at the user's explicit, informed request for personal use, with the CGU conflict above re-confirmed and consciously accepted rather than resolved (see ADR `011-nonograms-org-url-import-via-headless-browser`). A Puppeteer-based headless browser renders the puzzle page and reads clue numbers directly as HTML text from the interactive widget's cells — more accurate than the OCR path built for item 021, and confirmed against the same real puzzle OCR previously misread. Feeds the same logic-only solver as the image import; nothing about the "never trust the source for cells" guarantee changes.

...
## Acceptance Criteria
- [x] Saving a new nonogram creates a JSON file and returns/generates a stable id
- [x] Loading by id returns the previously saved grid, name, and timestamps unchanged
- [x] Listing returns all saved nonograms without requiring each one to be fully loaded individually
- [x] Deleting a nonogram removes its file and it no longer appears in listing or load results

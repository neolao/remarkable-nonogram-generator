...
## Acceptance Criteria
- [ ] In the live SVG preview, the border is visibly thicker every 5th row line and every 5th column line compared to regular cell borders
- [ ] In the generated/downloaded PDF, the same thicker line pattern appears every 5 rows and every 5 columns
- [ ] A grid smaller than 5 in a dimension shows only the regular, non-thickened borders (no thick line appears at the outer edge only)
- [ ] The thicker lines do not overlap or misalign with the regular cell grid, and clue numbers remain fully readable

...
**Code:** `authenticate()` in `packages/core/src/infrastructure/remarkable-auth.ts`
...
**Code:** `createNonogram()`, `Nonogram` in `packages/core/src/domain/nonogram-grid.ts`
...
**Code:** `computeNonogramClues()`, `NonogramClues` in `packages/core/src/domain/nonogram-clues.ts`
